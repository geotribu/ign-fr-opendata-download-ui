...
read -r -d '' DEP <<EOI
function attachPopups_ign_dep_$value_dep() {\\
ign_dep_$value_dep.eachLayer(function popUp (layer) {\\
...
read -r -d '' REGION <<EOI
function attachPopups_ign_reg_$value_region() {\\
ign_reg_$value_region.eachLayer(function popUp (layer) {\\
var props = layer.feature.properties;\\
var out = [];\\
if (props){\\
  for(var key in props){\\
...
read -r -d '' FR <<EOI
function attachPopups_ign_fr_$value_fr() {\\
ign_fr_$value_fr.eachLayer(function popUp (layer) {\\
var props = layer.feature.properties;\\
var out = [];\\
if (props){\\
  for(var key in props){\\
